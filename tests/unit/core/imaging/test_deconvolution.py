...
pytestmark = pytest.mark.skipif(not HOGBOM_AVAILABLE, reason="Hogbom extension not compiled/available")
...
class TestValidateDconvParams:
    ...
    def test_threshold_validation(self):
        ...
        for threshold in invalid_thresholds:
            params = {"threshold": threshold}
            with pytest.raises(ValueError, match="Threshold must be non-negative or None"):
                _validate_dconv_params(params)
    ...
    def test_clean_box_validation(self):
        ...
        for clean_box in invalid_clean_boxes:
            params = {"clean_box": clean_box}
            with pytest.raises(ValueError, match="Clean box must be a tuple of slices or None"):
                _validate_dconv_params(params)
...
class TestHogbomUtilities:
    ...
    @pytest.mark.skipif(not HOGBOM_AVAILABLE, reason="Hogbom extension not compiled/available")
...
class TestMaximgFunction:
    ...
    def test_maximg_mask_dimension_mismatch(self):
        ...
        with pytest.raises(RuntimeError, match="Mask dimensions must match image spatial dimensions"):
