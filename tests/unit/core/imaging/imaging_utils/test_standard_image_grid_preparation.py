...
class TestStandardImGridPrep(unittest.TestCase):
    def make_image_xds(self, npol=1, nchan=1):
        ...
        sky_data_shape = (len(epoch), len(freq), len(pol), imsize[0], imsize[1])
    ...
    def test_remove_padding(self):
        ...
        ra_o, dec_o = (a.right_ascension[99, 99].data, a.declination[99, 99].data)
        a = remove_padding(a, [180, 180])
        ra_f, dec_f = (a.right_ascension[89, 89].data, a.declination[89, 89].data)
    ...
    def test_mult_div(self):
        ...
        a["SKY"] = 1  # set all values to 1
        sky_dims = a["SKY"].dims
        sky_coords = a["SKY"].coords
        sky_shape = a["SKY"].shape
        a["PRIMARY_BEAM"] = xarray.DataArray(
            np.ones(sky_shape, dtype=np.float32), coords=sky_coords, dims=sky_dims
        )
        a["PRIMARY_BEAM"][0, 1, 0, :, :] = 0
        _mult_div(a, multiply=True)
        self.assertEqual(np.max(a["SKY"][0, 0, 0, :, :]), 1.0)
