...
class TestStandardDegrid(unittest.TestCase):
    def setup_vis_uvw(self):
        ...
        self.uvw[0, :, 0] = [
            random.uniform(-maxUV, maxUV) for i in range(nant * nant)
        ]
        self.uvw[0, :, 1] = [
            random.uniform(-maxUV, maxUV) for i in range(nant * nant)
        ]
    ...
    def test_sgrid_basic(self):
        ...
        phasor = sgrid(
            uvw, dphase, freq, c, scale, offset, sampling, pos, loc, off
        )
        ...
        expected_off_x = int(
            round((expected_loc_x - expected_pos_x) * sampling)
        )
        expected_off_y = int(
            round((expected_loc_y - expected_pos_y) * sampling)
        )
        expected_phase = -2.0 * np.pi * dphase * freq / c
        expected_phasor = complex(
            np.cos(expected_phase), np.sin(expected_phase)
        )
    ...
    def test_sgrid_numba(self):
        ...
        phasor, loc, off = sgrid_numba(
            uvw, dphase, freq, c, scale, offset, sampling
        )
