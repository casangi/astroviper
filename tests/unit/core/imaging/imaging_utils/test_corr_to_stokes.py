...
class TestXarrayCompatibility(unittest.TestCase):
    """Test that low-level functions work with numpy arrays extracted from xarray.

    Note: Low-level functions (corr_to_stokes, stokes_to_corr) accept only numpy arrays.
    For xarray DataArrays, use .values to extract the underlying array, or use the
    high-level functions (image_corr_to_stokes, image_stokes_to_corr) which handle
    xarray directly.
    """

    def test_xarray_linear(self):
        """Test linear conversion with numpy array extracted from xarray."""
        ...
        # Extract numpy array before passing to low-level function
    ...
    def test_xarray_circular(self):
        """Test circular conversion with numpy array extracted from xarray."""
        corr_xr = xr.DataArray(
