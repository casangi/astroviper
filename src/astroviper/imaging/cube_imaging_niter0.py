...
def cube_imaging_niter0(ps_name, image_name, grid_parms, n_chunks, data_variables=['sky','point_spread_function','primary_beam'], intents = ["OBSERVE_TARGET#ON_SOURCE"],compressor = Blosc(cname='lz4', clevel=5)):
    import numpy as np
    import xarray as xr
    import dask
    import os
    ...
    if isinstance(grid_parms["phase_direction"],int):
        ms_xds_name = summary_df.loc[summary_df["field_id"] == grid_parms["phase_direction"]].name.values[0]
        grid_parms["phase_direction"] = ps[ms_xds_name].attrs["field_info"]["phase_direction"]
    ...
    img_xds = make_empty_sky_image(xds=img_xds, phase_center=grid_parms['phase_direction']['data'],
        ...
        pol_coords=ms_xds.polarization.values, time_coords=[0],
    )


    write_image(img_xds,imagename=image_name,out_format='zarr')

    from xradio.image._util._zarr.zarr_low_level import write_binary_blob_to_disk, read_json_file, write_json_file, read_binary_blob_from_disk, pad_array_with_nans, create_data_variable_meta_data_on_disk


    xds_dims = dict(img_xds.dims)
    data_varaibles_and_dims_sel = {key: data_varaibles_and_dims[key] for key in  data_variables}
    zarr_meta = create_data_variable_meta_data_on_disk(image_name,data_varaibles_and_dims_sel,xds_dims,parallel_coords,compressor)
...
data_varaibles_and_dims= {'aperture':{'dims':full_dims_uv,'dtype':"<c16",'name':'APERTURE'},
                          'aperture_normalization':{'dims':norm_dims,'dtype':"<c16",'name':'APERTURE_NORMALIZATION'},
                          'primary_beam':{'dims':full_dims_lm,'dtype':"<f8",'name':'PRIMARY_BEAM'},

                          'uv_sampling':{'dims':full_dims_uv,'dtype':"<c16",'name':'UV_SAMPLING'},
                          'uv_sampling_normalization':{'dims':norm_dims,'dtype':"<c16",'name':'UV_SAMPLING_NORMALIZATION'},
                          'point_spread_function':{'dims':full_dims_lm,'dtype':"<f8",'name':'POINT_SPREAD_FUNCTION'},
                          
                          'visibility':{'dims':full_dims_uv,'dtype':"<c16",'name':'VISIBILITY'},
                          'visibility_normalization':{'dims':norm_dims,'dtype':"<c16",'name':'VISIBILITY_NORMALIZATION'},
