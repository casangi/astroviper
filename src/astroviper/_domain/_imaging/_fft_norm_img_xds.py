...
from astroviper._domain._imaging._check_imaging_parameters import _check_grid_parms, _check_norm_parms
import copy
...
def  _fft_norm_img_xds(img_xds, gcf_xds, grid_parms,norm_parms,sel_parms):
    ...
    data_group_in, data_group_out = _check_sel_parms(img_xds,_sel_parms,default_data_group_out={"imaging":{"primary_beam":"PRIMARY_BEAM","point_spread_function":"POINT_SPREAD_FUNCTION","sky_image":"SKY_IMAGE"}})
    fft_pair = {"aperture_grid":"primary_beam","uv_sampling_grid":"point_spread_function","visibility_grid":"sky_image"}
    sum_of_weight_pair =  {"aperture_grid":"aperture_grid_sum_weight","uv_sampling_grid":"uv_sampling_sum_weight","visibility_grid":"visibility_sum_weight"}
    ...
    for data_variable in ["aperture_grid","uv_sampling_grid","visibility_grid"]:
        if data_variable in data_group_in.keys():
            ...
            if data_variable == "aperture_grid":
                ...
                image = _remove_padding(_ifft_uv_to_lm(img_xds[data_group_in[data_variable]].values),_grid_parms['image_size']).real
                sum_weight_copy = copy.deepcopy(img_xds[data_group_out["aperture_grid_sum_weight"]].values)
                ...
                image = _remove_padding(_ifft_uv_to_lm(img_xds[data_group_in[data_variable]].values),_grid_parms['image_size'])
                sum_weight = img_xds[data_group_out[sum_of_weight_pair[data_variable]]].values
                ...
                image = _normalize(image, sum_weight, primary_beam, oversampling, ps_correcting_image, direction, norm_parms=_norm_parms)
            img_xds[data_group_out[fft_pair[data_variable]]] = xr.DataArray(image,dims=('frequency','pol','l','m'))
            
def _ifft_uv_to_lm(image):
    image_shape = image.shape
    return np.fft.fftshift(np.fft.ifft2(np.fft.ifftshift(image, axes=(2, 3)), axes=(2, 3)), axes=(2, 3)).real*(image_shape[-1]*image_shape[-2])
    
    #dafft.fftshift(dafft.ifft2(dafft.ifftshift(grids_and_sum_weights[0], axes=(0, 1)), axes=(0, 1)), axes=(0, 1))
    

def _fft_lm_to_uv(image):
    return np.fft.fftshift(np.fft.fft2(np.fft.ifftshift(image, axes=(2, 3)), axes=(2, 3)), axes=(2, 3)).real
...
def _normalize(image, sum_weight, primary_beam, oversampling, ps_correcting_image, direction, norm_parms):
    """
    ...
    if direction == 'forward':
        ...
        if norm_type == 'flat_noise':
            ...
            normalized_image = normalize_image(image, sum_weight[:,:,None,None], normalizing_image, oversampling, correct_oversampling)
            ...
            normalizing_image = ps_correcting_image[None,None,:,:]*primary_beam*primary_beam
            ...
            normalized_image = da.map_blocks(normalize_image, image, sum_weight[:,:,None,None], normalizing_image, oversampling, correct_oversampling)
