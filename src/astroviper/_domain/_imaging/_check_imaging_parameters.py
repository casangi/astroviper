...
def _check_grid_params(grid_params):
    import numbers
...
def _check_gcf_params(gcf_params):
    import numbers
    ...
    if not (
        ...
        params_passed = False
    if not (
        check_params(gcf_params, "chan_tolerance_factor", [numbers.Number], default=0.005)
    ):
        params_passed = False
    if not (
        check_params(
...
def _check_mosaic_pb_params(pb_mosaic_params):
    params_passed = True
...
def _check_rotation_params(rotation_params):
    import numbers
    ...
    if not (
        check_params(rotation_params, "common_tangent_reprojection", [bool], default=True)
    ):
        params_passed = False
...
def _check_norm_params(norm_params):
    import numbers
...
def _check_imaging_weights_params(imaging_weights_params):
    import numbers
    ...
    if imaging_weights_params["weighting"] == "briggs_abs":
        if not (
            check_params(
                imaging_weights_params, "briggs_abs_noise", [numbers.Number], default=1.0
            )
        ):
            params_passed = False
...
def _check_pb_params(img_dataset, pb_params):
    import numbers
    ...
    if len(pb_params["list_dish_diameters"]) != len(pb_params["list_blockage_diameters"]):
        print(
            "######### ERROR:Parameter ",
            "list_dish_diameters and list_blockage_diameters must be the same length.",
        )
