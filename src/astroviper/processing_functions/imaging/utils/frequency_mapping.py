...
def map_visibility_frequencies_to_image(
    ...
    *,
    ...
    """Map visibility channels one-to-one onto image-frequency channels.

    Parameters
    ----------
    visibility_frequencies : array-like
        One-dimensional visibility-channel frequencies.
    image_frequencies : array-like
        One-dimensional image-channel frequencies containing every visibility
        frequency exactly once.
    rtol : float, default 1e-12
        Relative tolerance used to match frequency coordinates.
    atol : float, default 0.0
        Absolute tolerance used to match frequency coordinates.

    Returns
    -------
    numpy.ndarray
        Integer image-plane index for each visibility channel.

    Raises
    ------
    ValueError
        If either coordinate is not one-dimensional and finite, if a
        visibility frequency does not match exactly one image frequency, or
        if multiple visibility frequencies map to the same image channel.
    """
