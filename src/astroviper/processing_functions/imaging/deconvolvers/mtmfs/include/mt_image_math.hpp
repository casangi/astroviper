...
// Small image reductions shared by the MTMFS engine and its Python binding.
//
...
// row-major (ny, nx) storage and never allocate.

#include <cstddef>

namespace mtmfs {

// Sum of all n elements of a (casacore sum(Array): accumulate in storage order
// with an accumulator of the array's own type).
template <typename T>
...
// The signed value and (ix, iy) position of the largest-magnitude element,
...
extern template PeakResult<double> find_max_abs_mask<double>(const double*, const double*, int,
                                                             int);
