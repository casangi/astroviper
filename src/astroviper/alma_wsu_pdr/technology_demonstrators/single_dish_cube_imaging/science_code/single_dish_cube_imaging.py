...
def single_dish_cube_imaging(
    ...
    # Select MSv4s
    info, msv4_selection = select_data(ps_xdt, data_selection)
    # Get Dask client
    client = get_client()
    # Schedule distributed computations of data=spectra spatial coordinates
    future_data_directions = {
        ...
        for antenna_name, msv4_dt in msv4_selection.items()
    }
    ...
    msv4_name, msv4_dt = next(iter(msv4_selection.items()))
    ...
    # Schedule distributed computations of single-dish image slices
    future_images_slices = []
    slices_info = []
    slice_index = 0
    for slice_start_channel in range(0, n_channels, channels_chunk):
        ...
        future_images_slices.append(
            client.submit(
                ...
                # data_directions=future_data_directions_arrays,
                future_data_directions=future_data_directions,
                pure=False
            )
        )
        slice_index += 1
    # Wait for completion
    # data_directions = client.gather(future_data_directions)
    images_slices = client.gather(future_images_slices)
    # Debug slices
    do_debug_slices = False
    slices_debug = {}
    if do_debug_slices:
        plain_image_path = image_kind_path["plain_image"]
        plain_image_store = zarr.open_array(plain_image_path, mode="r")
        slices_debug["store_shape"] = plain_image_store.shape
        slices_debug["debug_info"] = []
        for slice_info in slices_info:
            slice_channels_range = slice(
                slice_info["start_channel"], slice_info["end_channel"] + 1
            )
            slice_min = plain_image_store[:, :, slice_channels_range, :].min()
            slice_max = plain_image_store[:, :, slice_channels_range, :].max()
            slices_debug["debug_info"].append(
                {
                    "slice_index": slice_info["slice_index"],
                    "boundary_channels": [
                        slice_channels_range.start,
                        slice_channels_range.stop - 1
                    ],
                    "slice_min": f"{slice_min}",
                    "slice_max": f"{slice_max}"
                }
            )
    ...
    runtime_info = {
        ...
        "duration_s": duration,
        "channels_chunk": channels_chunk
    }
    with open(f"{image_store}/runtime_info.json", "w") as f:
    ...
    result = {
        ...
        "info": info,
        "slices_debug": slices_debug,
        "images_slices": images_slices,
        # "data_directions_debug": [
        #     v["debug_info"]
        #     for v in data_directions.values()
        # ],
        "selected_msv4_count": len(msv4_selection.keys()),
        "images_path": image_kind_path
    }
