...
def _check_logger_parms(logger_parms):
    ...
    if not (_check_parms(logger_parms, "log_to_term", [bool], default=True)):
        parms_passed = False
    if not (_check_parms(logger_parms, "log_to_file", [bool], default=False)):
        parms_passed = False
    if not (_check_parms(logger_parms, "log_file", [str], default="viper_")):
        parms_passed = False
    if not (
        _check_parms(
            logger_parms,
...
def _check_worker_logger_parms(logger_parms):
    ...
    if not (_check_parms(logger_parms, "log_to_term", [bool], default=False)):
        parms_passed = False
    if not (_check_parms(logger_parms, "log_to_file", [bool], default=False)):
        parms_passed = False
    if not (_check_parms(logger_parms, "log_file", [str], default="viper_")):
