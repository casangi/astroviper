...
def plot_astroviper_vs_casa_weights_interactive(ps_xdt, ms_xdt_name, robust, freq_idx, bl_idx):
    import matplotlib.pyplot as plt
    import numpy as np

    ms_xdt = ps_xdt[ms_xdt_name]
    av = ms_xdt[f"WEIGHT_AV_R_{robust}"].isel(baseline_id=bl_idx, frequency=freq_idx, polarization=0)
    casa = ms_xdt[f"WEIGHT_CASA_R_{robust}"].isel(baseline_id=bl_idx, frequency=freq_idx)
...
def plot_astroviper_vs_casa_weights_imshow_interactive(ps_xdt, ms_xdt_name, robust, bl_idx):
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    fig, (ax1, ax2, ax3) = plt.subplots(
        1, 3, figsize=(24, 8), constrained_layout=True
    )
